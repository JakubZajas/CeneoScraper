...
#opinions.score = opinions.scoer.map(lambda x: float(x[:-2].replace(",",".")))
...
#avg_score = opinions.scoer.map(lambda x: float(x.split("/")[0].replace(",","."))).mean()
avg_score = opinions.score.mean()

print(
    f'''Dla produktu  o kodzie: {product_code} dostepnych jest {opinions_count} opinii.
    Dla {pros_count} opinni dostepna jest  lista  zalet,
    a dla {cons_count} dostepna jest liczb wad.
    Srednia  ocena produktu to {avg_score.round(2)}.'''
)
